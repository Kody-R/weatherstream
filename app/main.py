...
renderer=WeatherRenderer(config_store,weather_manager,radar_manager,place_manager,history_store,spc_manager); streamer=Streamer(config_store,renderer)
...
app=FastAPI(title="WeatherStream / Roller Weather Network",version="0.1.8.1",lifespan=lifespan)
...
LIVE_DIR.mkdir(parents=True,exist_ok=True); app.mount("/live",StaticFiles(directory=str(LIVE_DIR)),name="live")
...
class SettingsRequest(BaseModel):
    ...
    storm_guidance:dict|None=None; spc:dict|None=None; history:dict|None=None; smart_programming:dict|None=None; dayparts:dict|None=None; cache:dict|None=None; channels:dict|None=None; video:dict|None=None
...
def api_update_settings(payload:SettingsRequest):
    updated=config_store.update_general(payload.model_dump(exclude_none=True)); weather_manager.request_refresh(); place_manager.request_refresh(); radar_manager.request_refresh(); spc_manager.request_refresh(); streamer.request_restart(); streamer.request_reconfigure(); return updated

@app.post("/api/locations")
def api_add_location(payload:ZipRequest):
    postal=payload.postal_code.strip()
    if not re.fullmatch(r"\d{5}",postal): raise HTTPException(status_code=400,detail="v0.1.8.1 accepts 5-digit U.S. ZIP codes.")
...
def api_status():
    ...
    return {
        "version":"0.1.8.1","network":{"name":settings.get("station_name"),"callsign":settings.get("station_callsign")},
        ...
        "sources":sources,"cache":{**cache_manager.status(),"retention_hours":(settings.get("cache") or {}).get("retention_hours",48)},"stream":streamer.status(),"configured_locations":len(settings.get("locations",[])),
    }
...
async def upload_branding_logo(request:Request):
    ...
    BRANDING_DIR.mkdir(parents=True,exist_ok=True); tmp=BRANDING_LOGO.with_suffix(".tmp.png"); image.save(tmp,"PNG",optimize=True); tmp.replace(BRANDING_LOGO); streamer.request_restart(); return {"ok":True,"width":image.width,"height":image.height}
@app.delete("/api/branding/logo")
def delete_branding_logo(): BRANDING_LOGO.unlink(missing_ok=True); streamer.request_restart(); return {"ok":True,"using_builtin":True}
...
def playlist(request:Request):
    ...
    for spec in channel_specs(settings):
        lines.append(f'#EXTINF:-1 tvg-id="{spec["id"]}" tvg-name="{spec["name"]}" tvg-logo="{logo}" group-title="Roller Weather Network",{spec["name"]}')
...
@app.get("/health")
def health(): return {"status":"ok","version":"0.1.8.1","stream":streamer.status()}
