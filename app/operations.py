...
def create_backup_bytes(settings: dict[str, Any]) -> bytes:
    ...
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        zf.writestr("manifest.json", json.dumps({"product":"WeatherStream","version":"0.2.6","created_at":time.time()}, indent=2))
...
def create_diagnostics_bytes(settings: dict[str, Any], app_status: dict[str, Any], channels: dict[str, Any], streamer) -> bytes:
    ...
    buf = io.BytesIO()
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        zf.writestr("README.txt", "WeatherStream v0.2.6 diagnostic bundle. No music files are included.\n")
